...
calc_quantile_splinebins <- function(p, data_pnad, groups = NULL){
        ...
        #data_i <- data_split[[7]]
        quantile_splinebins = function(p, data_i = data_i){

                limites  <- c(data_i$min_faixa[1], data_i$max_faixa)
                contagem <- c(0, data_i$n)
                fit      <- splinebins(bEdges = limites, bCounts = contagem)

                quantiles_i = tableInequality:::quantile(p , CDF_func = fit$splineCDF, max_x = fit$E)
